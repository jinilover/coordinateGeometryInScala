...
class BoundaryConditionSpec extends FlatSpec with Matchers {
  it should ("combine with 2 matching edges to form polygon has a hole, ok since all matching polygon edges are continuous") in {
    combine {
      Polygon(
  ...
  it should ("combine with 1 matching edges to form polygon has a hole, ok since all matching polygon edges are continuous") in {
    combine {
      Polygon(
        (10, 14), (16, 14), (16, 4), (3, 4),
        (3, 10), (7, 10), (7, 6), (13, 6),
  ...
  it should ("combine a polygon has a 'L' hole with a box, it's combineable or not depends on the box's posiiton") in {
