...
Mathemtical problem is a good domain for applying FP.  Since FP is used, the application is written in a simpler and more elegant way than using other programming paradigm.  In the implementation, only basic FP features are used such as higher-order functions, Monad.  Scalaz is used in a few places only.  Therefore in writing this mathematical solution, implementation takes the least time.  Most of the time is spent on mathematical analysis, i.e. using the graph book to draw various edges and polygons, and writing test cases.
